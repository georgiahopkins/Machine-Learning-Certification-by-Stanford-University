...
X = [ones(m,1) X]; %adding column of ones to X (adds bias)
z2 = Theta1*X';
a2 = sigmoid(z2);
a2 = [ones(m,1) a2']; %add column of ones to a2 (adds bias)
z3 = Theta2*a2';
a3 = sigmoid(z3);
h = a3;
...
%               first time.
%
%Back Propagation (steps from summary in notes)
for t=1:m
  
% Step 1 - set a1:=x(t)
  
	a1 = X(t,:); % already added bias to X with column of ones
  
%Step 2 - forward prop to compute a(l) up to L value (3) 
   a1 = a1'; 
	z2 = Theta1 * a1; 
	a2 = sigmoid(z2); 
    ...
    a2 = [1 ; a2]; % adding bias 
	z3 = Theta2 * a2;
	a3 = sigmoid(z3); % final activation layer a3 == h 
    
% Step 3 & 4- computing delta(L) and delta L-1 till reach L-2
...
% Step 5 - updating new delta matrix 
